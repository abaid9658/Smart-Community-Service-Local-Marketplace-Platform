...
import { serviceAPI, categoryAPI, favoriteAPI } from '@/lib/api';
import { Service, Category, Favorite } from '@/types';
import { useAuthStore } from '@/store/auth.store';
import { buildQueryString } from '@/lib/utils';
import {
  SlidersHorizontal, Search, Briefcase,
  X, Grid3X3, List, Loader2
} from 'lucide-react';
...
function ServicesContent() {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated) {
      favoriteAPI.list().then(({ data }) => {
        const favs: Favorite[] = data.data || [];
        const ids = new Set<string>(favs.filter(f => f.serviceId).map(f => f.serviceId as string));
        setFavorites(ids);
      }).catch(() => {});
    }
  }, [isAuthenticated]);
  ...
  const fetchServices = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await serviceAPI.list({ ...filters, limit: 12 });
  ...
  const updateFilter = (key: string, value: unknown) => {
    ...
    router.push(`/services?${buildQueryString(newFilters)}`);
  };
  ...
  const handleFavorite = async (serviceId: string) => {
    ...
    try {
      await favoriteAPI.toggle({ serviceId });
      setFavorites(prev => {
  ...
  const filterFields = (isMobile: boolean) => (
    <>
      ...
      {/* Price/Budget */}
      <div className="mb-5">
        <label className="text-xs font-bold text-gray-700 mb-2 block">Budget (PKR)</label>
    ...
    <div className="min-h-screen flex flex-col" style={{ background: '#FAF9FD' }}>
      ...
      <div className="bg-white border-b border-gray-100 py-8">
        <div className="max-w-[1280px] mx-auto px-6">
          <h1 className="text-3xl font-black mb-1.5 text-gray-900">Service Marketplace</h1>
          <p className="text-gray-500 text-sm">Hire {total.toLocaleString()}+ professional service providers in your city</p>
        </div>
      </div>
            ...
            <div className="fixed inset-0 z-50 lg:hidden flex">
              ...
              <aside className="relative w-80 max-w-full bg-white h-full p-6 overflow-y-auto flex flex-col z-10 shadow-2xl animate-fade-left">
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100">
                  <h2 className="text-lg font-black text-gray-900">Filter Services</h2>
